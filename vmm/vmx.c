...
// CHANGED FOR LAB 0
// Added the patch for this part - sharedon Piazza post 161
void asm_vmrun(struct Trapframe *tf)
{
	...
	asm(
		...
		: "cc", "memory", "rax", "rbx", "rdi", "rsi", "r8", "r9", "r10", "r11", "r12", "r13", "r14", "r15");
	lock_kernel();
	if (tf->tf_es)
	{
...
void msr_setup(struct VmxGuestInfo *ginfo)
{
...
void bitmap_setup(struct VmxGuestInfo *ginfo)
{
